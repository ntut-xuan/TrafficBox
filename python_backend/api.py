...
def NearByStation():
    ...
    bus_station_data = requests.get("https://maps.googleapis.com/maps/api/place/nearbysearch/json?&language=zh-TW&location=%f,%f&rankby=distance&types=transit_station&key=%s" % (lat, lng, key))
    bus_station_set = []
    result = []
    ...
    for object in bus_station_json_object["results"]:
        name = object["name"]
        if name in bus_station_set:
            continue
        bus_station_set.append(name)
    ...
    for name in bus_station_set:
        ...
        for stationID in bus_data_by_name[name].keys():
            ...
            result.append({"name": name, "stationID": stationID, "lat": bus_data_by_name[name][stationID]["lat"], "lng": bus_data_by_name[name][stationID]["lng"], "stops": stops})
